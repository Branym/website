...
function RootLayoutInner({ children, invert }) {
  let panelId = useId()
  ...
  return (
    <MotionConfig transition={shouldReduceMotion ? { duration: 0 } : undefined}>
      ...
      >
        ...
        >
          <GridPattern
            className="absolute inset-x-0 -top-14 -z-10 h-[1000px] w-full fill-neutral-900 stroke-neutral-800/50 [mask-image:linear-gradient(to_bottom_left,white_40%,transparent_50%)]"
            yOffset={-96}
            interactive
          />
...
export function RootLayout({ children, invert=false }) {
  let pathname = usePathname()
  let [logoHovered, setLogoHovered] = useState(false)

  return (
    <RootLayoutContext.Provider value={{ logoHovered, setLogoHovered }}>
      <RootLayoutInner invert={invert} key={pathname}>{children}</RootLayoutInner>
